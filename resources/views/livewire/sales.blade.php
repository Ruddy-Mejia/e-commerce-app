            ...
            <div class="p-4 sm:p-8 space-y-8">
                @foreach ($statuses as $status)
                    <div class="space-y-4">
                        ...
                        <div class="flex gap-4 overflow-x-auto pb-2" wire:poll.1s>
                                ...
                                <div
                                    ...
                                    <div class="p-4 overflow-y-auto h-full">
                                        <div class="flex justify-between">
                                            <p class="text-xl sm:text-xl font-semibold mb-2">{{ 'Order #' . $order->id }}</p>
                                            @if ($status === 'pending')
                                                <button class="border-4 border-green-400 hover:bg-gray-400 text-xs rounded-full px-3" wire:click='toogleStatus({{$order->id}})'>Complete</button>
                                            @endif
                                        </div>
                                        <p class="text-lg sm:text-lg mb-2">{{ 'Buyer name: ' . $order->user->name . ' ' . $order->user->lastname }}</p>
